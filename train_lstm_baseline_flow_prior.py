...
if __name__ == "__main__":
    ...
    model = LSTMVAE(args.hidden_size, args.latent_size, args.depth,
                    ...
                    use_flow_prior=args.use_flow_prior, use_aux_regressor=False).to(device)
    print(model)
    ...
    for epoch in range(epoch_to_start, args.epoch + 1):
        ...
        for batch in loader:
            ...
            if total_step % args.print_iter == 0:
                ...
                print(
                    "[%d] Beta: %.4f, Entropy: %.2f, Neg_log_prior: %.2f, KL: %.2f, Stop symbol: %.2f, Nucleotide symbol: %.2f, Structural symbol: %.2f, All symbol: %.2f, PNorm: %.2f, GNorm: %.2f" % (
